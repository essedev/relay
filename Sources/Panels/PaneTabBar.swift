...
public struct PaneTabBarActions {
    ...
    /// Spegne la sessione della tab tenendo il suo resume binding.
    public var deactivateTab: (WorkspaceModel.Tab) -> Void
    ...
    public init(
        ...
        deactivateTab: @escaping (WorkspaceModel.Tab) -> Void
    ) {
        self.deactivateTab = deactivateTab
        self.newTab = newTab
        ...
        return HStack(spacing: Theme.Spacing.xs) {
            ForEach(Array(tabs.enumerated()), id: \.element.id) { index, tab in
                PaneTabItem(
                    ...
                    // La selezionata del pane e' a schermo: buttarle la surface sotto lascerebbe
                    // un terminale morto. Le altre della strip hanno la sessione viva ma nessuna
                    // view, che e' esattamente il caso che costa memoria.
                    canDeactivate: tab.deactivationBlock(
                        isOnScreen: tab.id == pane.selectedTabID
                    ) == nil,
                    colors: colors,
                    ...
                    onDeactivate: { actions.deactivateTab(tab) },
