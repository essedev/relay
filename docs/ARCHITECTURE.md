...
- `AppSettings` (`WorkspaceModel`, @Observable) tiene tema selezionato + dimensione font + font
  family + blink del caret + preferenze notifiche, persistiti in `UserDefaults` (preferenze, non lo
  snapshot del layout). `fontSize`, `fontName` e `cursorBlink` sono sovrapposti al tema base
...
- Sei temi curati (dato puro in `Core`): scuri **Relay Dark** (One Dark), **Solarized Dark**,
  **Gruvbox Dark**; chiari **Relay Light** (One Light), **Solarized Light**, **Gruvbox Light**.
...
- sei temi curati (Solarized/Gruvbox oltre a Relay Dark/Light) e scelta font family.
