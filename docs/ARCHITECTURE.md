...
Pannello impostazioni (`Cmd+,`): categorie (Appearance / Terminal) come segmented themed + campo di
ricerca che filtra per parole chiave. Ogni voce è un "blocco" dichiarativo (categoria + keywords +
vista), unica fonte per categorie e ricerca: aggiungere un'impostazione è una riga. Anche zoom
(`Cmd +/-`, `Cmd+0`). Import da config Ghostty: possibile in futuro, non nel baseline.
