...
Cicli dall'analisi iniziale al primo giro di UI/UX e tooling, spostati qui dalla rotazione di
`../CYCLES.md` (che tiene gli ultimi ~15). Numerazione intatta: `Cycle 5` resta `Cycle 5`.
Le decisioni ancora vincolanti che nascono qui (engine SwiftTerm, boundary dei moduli, budget di
performance) vivono in `docs/ARCHITECTURE.md`, non solo in questo storico.
